...
Static HTML/CSS and compiled JavaScript are embedded in the launcher at build time. Browser application source is maintained in TypeScript under `cmd/launcher/ui`; `cmd/launcher/web/*.js` is generated build output. The complete Browser TypeScript surface is compiled and checked with `strict: true`; `noCheck` is not used. Shared declarations in `global.d.ts` define the `TLStudioKernel`, application state, required DOM elements, runtime/session/tool/live-event contracts, and extension-owned state instead of exposing `window.KLU` as `any`. The normal TL Studio control surface talks only to the launcher origin.
