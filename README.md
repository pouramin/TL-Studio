...
- **Live Preview** — local static HTML or Node dev-server preview in a movable/resizable browser window; root `index.html` is automatic, the active HTML file is preferred next, a single discovered HTML file is opened automatically, and multiple HTML files can be selected in the Preview panel.
