...
- **Live Preview** — capability-driven local preview in a movable/resizable browser window. TL Studio follows the active previewable file across HTML, SVG/raster images, PDF, video, audio, rendered Markdown, and plain text; Node projects still use their `package.json` dev server for HTML/app preview. Previewable binary media opens as a real read-only Workspace tab so PDF/image/media selection can drive Preview without forcing binary content into Monaco.
